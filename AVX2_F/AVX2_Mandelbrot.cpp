...
void count_Mandelbrot (SRender* render, Image* image)
{
    if (render->SIMD_mode == true)
    {
        for (int y = 0; y < render->H; y++)
        {
            ...
            double start_y  = render->max_Y - (double) y * delta_y;
            ...
            for (int x = 0; x < render->W; x += 4) //4 is hardcoded everywhere
            {
                ...
                double start_x  = render->min_X + (double) x * render->delta_X / ((double) render->W);

                __m256d x_0 = _mm256_add_pd (_mm256_set1_pd (start_x),_mm256_mul_pd (_mm256_set1_pd (delta_x), _mm256_set_pd (3, 2, 1, 0)));
                ...
                {
                    __m256d squared_x   = _mm256_mul_pd (cur_x, cur_x);
                    __m256d squared_y   = _mm256_mul_pd (cur_y, cur_y);

                    __m256d doubled_xy  = _mm256_mul_pd (cur_x, cur_y);
...
sf::Color paint_it (SRender* render, long int n)
{
    ...
    switch (render->color)
    {
        case stripes:
            ...
            return sf::Color{tone, tone, tone};
...
void no_sse_count_Mandelbrot (SRender* render, Image* image)
{
    ...
    {
        double y_0   = render->max_Y - render->delta_Y * ((double) y / (double) render->H);
        ...
        for (unsigned int x = 0; x < render->W; x++)
        {
            ...
            unsigned int cnt = 0;
            for (; (cnt <= render->max_cnt) && (x2 + y2 < render->squared_distance); cnt++, n++)
            {
                x2  = x_i * x_i;
                y2  = y_i * y_i;
                xy  = x_i * y_i;
            ...
            if (render->draw_permission == true)
            {
                image->setPixel(x, y, paint_it (render, n));
            }
        }
    }
...
void user_interruption (FUNC_ARGUMENTS, Window* window)
{
    ...
    if (event->type == Event::KeyPressed)
    {
        ...
        if (event->key.code == Keyboard::O) //OPTIMIZATION
        {
            change_mode (render);
        }

        if (event->key.code == Keyboard::C) //COLOR
        {
            if (render->color == stripes)
            {
                render->color = black_n_white;
                return;
            }

            if (render->color == black_n_white)
            {
